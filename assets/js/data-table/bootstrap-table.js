    ...
    if (index !== -1) {
      ...
      if (this.options.sortClass !== undefined) {
        ...
        timeoutId = setTimeout(function () {
          ...
          var index = that.$header.find(
            sprintf('[data-field="%s"]', that.options.sortName).index() + 1
          );
          that.$el
            .find(sprintf("tr td:nth-child(%s)", index))
    ...
    if (this.options.sidePagination !== "server") {
      ...
      this.data = f
        ? $.grep(this.options.data, function (item, i) {
            for (var key in f) {
              if (
                ($.isArray(f[key]) && $.inArray(item[key], f[key]) === -1) ||
        ...
        ? $.grep(this.data, function (item, i) {
            for (var j = 0; j < that.header.fields.length; j++) {
              ...
              var key = $.isNumeric(that.header.fields[j])
                ? parseInt(that.header.fields[j], 10)
      ...
      if (this.options.pageSize === this.options.formatAllRows()) {
        ...
        var pageLst =
          typeof this.options.pageList === "string"
            ...
            : this.options.pageList;
        if (
          $.inArray(this.options.formatAllRows().toLowerCase(), pageLst) > -1
        ) {
          $allSelected = true;
        }
      }
  ...
  BootstrapTable.prototype.fitHeader = function () {
    ...
    this.$body
      ...
      .each(function (i) {
        ...
        var $th = that.$header_.find(
          sprintf('th[data-field="%s"]', visibleFields[index])
        );
        if ($th.length > 1) {
  ...
  BootstrapTable.prototype.resetFooter = function () {
    ...
    $.each(this.columns, function (i, column) {
      ...
      html.push(
        "<td",
        class_,
        sprintf(' style="%s"', falign + valign + csses.concat().join("; ")),
        ">"
      );
